...
JavaScript上では、日時のカラムから取得されたDate型の値は、ローカルタイムゾーンが適用された状態になります。
各データベースエンジンにおける保存形式はそれぞれ以下のようになります。

### SQLite

- データベース上ではUnix Timestampとして保存されます。(通常はタイムゾーンを意識する必要はありません)

### MySQL

- デフォルトでは、データベースに保存された時刻をUTCとして扱います。
- 接続時にtimezoneを指定することで、この挙動を変更することができます。
...
- timestamp (without time zone)型の場合、データベースに保存された時刻はローカルタイムゾーンとして扱われます。
- ローカルタイムゾーン以外で時刻を扱いたい場合は timestamp with time zone型の利用などを検討してください。
