...
This file is a modified version of the original Rails UJS implementation.
https://github.com/rails/rails/blob/main/actionview/app/javascript

All modifications are also released under the MIT license.
Original work Copyright (c) Rails Contributors
Modified work Copyright (c) Accella Contributors
 */

/* eslint-disable */
// @ts-nocheck
const linkClickSelector =
...
const handleConfirmWithAccel = (accel) =>
  function (e) {
    if (!allowAction(this, accel)) {
      stopEverything(e);
    }
  };
...
var allowAction = function (element, accel) {
  let callback;
  ...
  if (fire(element, "confirm")) {
    try {
      answer = accel.confirm(message, element);
    } catch (error) {}
...
var enableFormElements = (form) =>
  formElements(form, formEnableSelector).forEach((element) => enableFormElement(element));
...
const handleMethodWithAccel = (accel) =>
  function (e) {
    const link = this;
...
const handleRemoteWithAccel = (accel) =>
  function (e) {
    let data, method, url;
    const element = this;
    if (!isRemote(element)) {
    ...
    if (matches(element, formSubmitSelector)) {
      ...
      method =
        getData(element, "ujs:submit-button-formmethod") || element.getAttribute("method") || "get";
      url =
...
const start = function () {
  if (window._ujs_loaded) {
    throw new Error("accel-ujs has already been loaded!");
  }
  ...
  return (window._ujs_loaded = true);
};
...
const handleConfirm = handleConfirmWithAccel(Accel);

Accel.handleConfirm = handleConfirm;

const handleMethod = handleMethodWithAccel(Accel);

Accel.handleMethod = handleMethod;

const handleRemote = handleRemoteWithAccel(Accel);

Accel.handleRemote = handleRemote;

export { Accel as default };
